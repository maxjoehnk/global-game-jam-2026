...
const CutBody = preload("res://Scenes/Physics/cutable_ridgid_body.tscn")
const DefaultColor = Color(0.8, 0.0, 0.224, 0.392) 
const HoverColor = Color(0.025, 0.522, 0.0, 0.196) 
...
func build_new_object(new_polygons, freeze_object: bool = true):
	var parent = self.physics_body.get_parent()
	for poly_new in new_polygons:
		var point_center = points_center(poly_new)
		poly_new = self.shift_polygon_vec(-point_center, 0.0, poly_new)
		var new_body = self.CutBody.instantiate()
